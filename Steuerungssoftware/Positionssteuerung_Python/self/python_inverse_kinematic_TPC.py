...
def inverse_kinematik(old_x, old_y, old_thetta, new_x, new_y, new_thetta):
    ...
    x_4_old=x_tcp_old-math.sin(math.radians(old_thetta))*l4
    y_4_old=y_tcp_old+math.cos(math.radians(old_thetta))*l4
    ...
    alpha_groß_old=math.atan2(x_4_old,(y_4_old-y_2))         #für acos eher atan2 nehmen, dann auch negative werte
    ...
    gamma_old=math.acos(((-1)*math.pow(l2,2)+math.pow(l3,2)+math.pow(l24_old,2))/(2*l3*l24_old))

    #epsilon_old=math.atan2(x_4_old,y_4_old)
    epsilon_old=math.asin(x_4_old/l24_old)

    thetta_gamma_old=(math.pi/2)-epsilon_old-gamma_old

    thetta_epsilon_old=math.radians(old_thetta)+epsilon_old+gamma_old

    if(x_4_old<x_1):
        epsilon_old=math.pi-epsilon_old

    #Berechnung TCP zu Joint 4 (OLD)
    x_tcp_new=new_x
    y_tcp_new=new_y

    x_4_new=x_tcp_new-math.sin(math.radians(new_thetta))*l4
    y_4_new=y_tcp_new+math.cos(math.radians(new_thetta))*l4
    ...
    alpha_groß_new=math.atan2(x_4_new,(y_4_new-y_2))
    ...
    gamma_new=math.acos(((-1)*math.pow(l2,2)+math.pow(l3,2)+math.pow(l24_new,2))/(2*l3*l24_new))

    #epsilon_new=math.atan2(x_4_new,y_4_new)
    epsilon_new=math.asin(x_4_new/l24_new)
    
    if(x_4_new<x_1):
        epsilon_new=math.pi-epsilon_new

    

    thetta_gamma_new=(math.pi/2)-epsilon_new-gamma_new

    thetta_epsilon_new=math.radians(new_thetta)+epsilon_new+gamma_new

    #Berechnung der Differenz
    delta_alpha=alpha_2_old-alpha_2_new     #weil Winkel nicht im Dreieck sondern auserhalb (daher old-new)
    delta_beta=beta_new-beta_old

    delta_epsilon_thetta=thetta_epsilon_new-thetta_epsilon_old

    #delta_thetta_2=math.radians(new_thetta)-math.radians(old_thetta)
    delta_thetta_gamma=math.radians(thetta_gamma_old)-math.radians(thetta_gamma_new)
    ...
    #joint 4: 13000 Stepps für 360°
    thetta_stepps=(13000/(2*math.pi))*(delta_epsilon_thetta)
    #thetta_stepps=(13000/(2*math.pi))*(delta_thetta_gamma)

    print_from_py("Berechnung beendet, alpha um "+str(math.degrees(delta_alpha)) +"° verfahren und beta um "+str(math.degrees(delta_beta)) +"° verfahren und epsilon "+str(math.degrees(delta_epsilon_thetta)) +"° verfahren!")
    return alpha_stepps, beta_stepps, thetta_stepps
    ...
    #       [Joint 1 (rotation): 5500Stepps := 90°]" [Joint 2: 8200Stepps := 90°]" Joint 3:[6300Stepps := 90°]" Joint 4:[3250Stepps := 90°]"
...
arduino_port = 'COM3'  # Portangabe Windows
#arduino_port = '/dev/ttyUSB0'  # Portangabe Linux
baud_rate = 9600
...
while(True):
    ...
    if(stepindex==1):       #manuell mode
        ...
        alpha_stepps, beta_stepps, thetta_stepps=inverse_kinematik(int(old_x_from_terminal),int(old_y_from_terminal),int(old_thetta_from_terminal),int(x_from_terminal),int(y_from_terminal), int(new_thetta_from_terminal))      #UNBEDINGT noch 0,0 zu x_old und y_old anpassen!

        print_from_py("AlphaStepps:" +str(alpha_stepps)+", BetaStepps: "+str(beta_stepps)+", Thetta Stepps: "+str(thetta_stepps))
