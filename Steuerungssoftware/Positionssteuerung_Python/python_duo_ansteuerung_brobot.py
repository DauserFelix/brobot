...
def select_joint():
    ...
    while True:
        try:
            user_input = input("\nWelches Joint möchten Sie ansteuern? (1 oder 2): ").strip()
    ...
    while (time.time() - timeout_start) < 5:
        try:
            ...
            if (current_joint == 1 and message == Code1) or (current_joint == 2 and message == Code2):
                print(f" {message}")
...
def change_joint():
    ...
    print("Verfügbare Joints: 1, 2")
    
    try:
        new_joint = input("Neues Joint auswählen (1 oder 2, Enter zum Abbrechen): ").strip()
