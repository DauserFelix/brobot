...
Code4 = "Arduino: Joint 4 wird angesteuert"
...
def select_joint():
    ...
    print("3 - Joint 3 (Pins 5,6,7)")
    print("4 - Joint 4 (Pins 2,3,4)")
    print("=" * 50)
    
    while True:
        try:
            user_input = input("\nWelches Joint möchten Sie ansteuern? (1, 2, 3 oder 4): ").strip()
            
            if user_input in ['1', '2', '3', '4']:
                current_joint = int(user_input)
                send_data(current_joint)  # Sende Byte-Wert für Joint
                break
            else:
    ...
    while (time.time() - timeout_start) < 5:
        try:
            ...
            if ((current_joint == 1 and message == Code1) or 
                ...
                (current_joint == 3 and message == Code3) or
                (current_joint == 4 and message == Code4)):
                print(f" {message}")
...
def change_joint():
    ...
    print("Verfügbare Joints: 1, 2, 3, 4")
    
    try:
        new_joint = input("Neues Joint auswählen (1, 2, 3 oder 4, Enter zum Abbrechen): ").strip()
        
        if new_joint == '':
            return
        elif new_joint in ['1', '2', '3', '4']:
            current_joint = int(new_joint)
            send_data(current_joint)
        else:
...
def show_joint_info():
    ...
    print("Joint 3: Pins 5 (STEP), 6 (DIR), 7 (ENA)")
    print("Joint 4: Pins 2 (STEP), 3 (DIR), 4 (ENA)")
    print("=" * 50)
...
def main():
    ...
    print("\n" + "=" * 50)
    print("POSITIONSSTEUERUNG - 4 JOINT SYSTEM")
    print("=" * 50)
